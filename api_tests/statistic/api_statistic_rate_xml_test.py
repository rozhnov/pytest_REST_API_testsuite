...
def setup_module():
    ...
    sender = Sender()
    request = sender.get("/api/rest/webmaster/xml/statistics-rate/event_time_day/2014-12-01/2014-12-20?")
...
def test_statistic_rate_xml_status():
    assert code == 200
...
def test_statistic_rate_xml_data():
    ...
    if status == 200:
        ...
        for item in items:
            ...
            assert len(item.find('eventTime').text) >= 0
